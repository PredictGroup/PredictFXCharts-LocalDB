...
namespace PredictFXCharts
{
    public class ValueAtRisk_Level : DemoModule
    {
        ...
        double[] ValueAtRisk_Data = null;



        //Name of demo module
        public string getName() { return "Value-At-Risk"; }
        ...
        protected bool getData(DateTime startDate, DateTime endDate)
        {
            ...
            get3MinDataExt(startDate, endDate);
        ...
        /// <param name="startDate">The starting date/time for the data series.</param>
        /// <param name="endDate">The ending date/time for the data series.</param>
        protected void get3MinDataExt(DateTime startDate, DateTime endDate)
        {
            ValueAtRisk_Data = new Double[VolQuotesLst.Count()];

            int i = 0;
            foreach (volquotes s in VolQuotesLst)
            {
                var valueAtRisk = (from v in ValueAtRiskLst
                                   where v.volquoteid == s.id
                                   select v).FirstOrDefault();
                if (valueAtRisk == null)
                    ValueAtRisk_Data[i] = 0;
                else
                    ValueAtRisk_Data[i] = valueAtRisk.value.Value;
